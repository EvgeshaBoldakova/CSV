const cities = `44.38,34.33,Алушта,31440,
...
function processCSV(cities) {
    const newCities = cities
        ...
        .reduce((acc, city, index) => {
            acc[city.name] = {population: +city.population, rating: (10 - index)};
            return acc;
        }, {});

    return (text) => {
        const regex = new RegExp(`(^|\\s)(${Object.keys(newCities).join('|')})(\\s|[., ;?!]|$)`, 'g');

        return text.replace(regex, (match, before, city, after) => {
            const {population, rating} = newCities[city];
            return `${before}${city} (${rating} місце в ТОП-10 найбільших міст України, населення: ${population} ${
                declension(population)})${after}`;
        });
    };
}

function declension(population) {
    if (population % 10 === 1 && population % 100 !== 11) {
        return "людина";
    } else if ((population % 10 === 2 || population % 10 === 3 || population % 10 === 4)
            && population % 100 !== 12 && population % 100 !== 13 && population % 100 !== 14) {
            return "людини";
    } else {
        return "людей";
    }
}
console.log(processCSV(cities)(prompt("Enter text", "Наша столиця - Київ! Харків - велике місто, як і Дніпро. А як щодо Запоріжжя?")))